...
natoms = length(xa);
xa = xa + (rand(natoms,1)-0.5)*halfwave/4;
ya = ya + (rand(natoms,1)-0.5)*halfwave/4;


fname = ['./data/nx' int2str(nx) '_ny' int2str(ny) '_dk' num2str(dk) '.mat'];
...
saveas(gcf,join(['./results/irreg_nx' int2str(nx) '_ny' ...
     int2str(ny) '_xdir_msign.pdf']));
 fname_save = ['./data/everything_irreg_nx' int2str(nx) '_ny' int2str(ny) '_xdir_msign.mat'];
save(fname_save);
